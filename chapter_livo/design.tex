...
%
The cost of these sensors has resulted in the slower growth of RGB-D camera depth resolution.
...
\begin{itemize}
\item It uses two video decoders (running in parallel), which decode the compressed tiled frames to generate the distorted tiled color and depth frames.
...
%
For each pixel in each image, it must determine if that pixel falls within the frustum or not, and replace culled pixels with a zero value (both for color and depth).
%
