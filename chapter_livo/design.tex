...
\cref{fig:architecture} pictorially depicts the design of \livo's sender-to-receiver pipeline; in the deployment model above, each site runs one instance of the pipeline in each direction.
%
...
%
The frustum represents the receiver's 3D field of view (\cref{fig:architecture}).
%
...
%
But, for example, if she moves closer to objects or participants in the scene, her frustum will likely include a much smaller part of the point cloud (\cref{fig:architecture}). 
...
%
Consider a static RGB-D camera array (\cref{fig:architecture}) with $N$ frame-synchronized cameras.\footnote{There exist techniques to synchronize Kinect cameras~\cite{kinect-sync}.}
%
...
\begin{figure}[t]
  \centering \includegraphics[width=\textwidth]{figures/color_depth_rmse_vs_split_bwe80000_logscale.pdf}
...
%
\livo{} continuously adapts the split using the following steps (\cref{fig:architecture}):
\begin{itemize}
...
%% %
%% We find (\cref{s:valid-design-choic}) that across different videos, quality saturates at about 150 bits per second per point for depth and at about a 1/7 of that for color.


%% % \begin{figure}[t]
%% %   \centering \includegraphics[width=0.8\columnwidth]{figures/depth_distortion.png}
...
%%   \begin{figure}[t]
%%     \centering \includegraphics[width=\columnwidth]{figures/depth_distortion.png}
%%     \caption{Depth distortion vs depth bitrate.}
%%     \label{plot:depth-distortion}
%%   \end{figure}
