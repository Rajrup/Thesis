...
%
Our \livo implementation (\cref{fig:architecture}) contains 15K lines of C++ and few hundred lines of Unity.
%
...
We implemented a prototype of \livo in C++ and Python with \note{XXX} lines of code on Ubuntu OS. As shown in~\cref{fig:architecture}, the \livo can be broadly divided into 2 major components -- Sender and Receiver. The key components of the Sender are:
\squishlist
...
\parae{Trace replay.}
This reads RGB-D frames from disk at 30~fps and feeds them into the \livo pipeline at the sender (\cref{fig:architecture}).
%
...
%
Starline streams 2D videos at a fixed quality, but in \livo-NoCull we include bandwidth splitting (\cref{ss:bw-adaptation}) and bandwidth-adaptivity (in \cref{s:valid-design-choic}, we also quantify what happens if we disable bandwidth-adaptivity).
%
