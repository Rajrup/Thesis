...
%
There exist several three-dimensional frame representations; we focus on \emph{point clouds}~\cite{vivo, groot} due to their popularity, but overview alternate representations in \S\ref{s:related}.
%
Each point in a point cloud corresponds to a 3D location in physical space (usually the surface of an object) and is associated both with location coordinates and color\footnote{Some sources of volumetric video, like LiDAR, do not produce color. We do not consider these.}
...
%
Commodity RGB-D cameras, such as Azure Kinect~\cite{kinect} or Intel RealSense~\cite{realsense}, can capture both the color (RGB) and depth (D) of points in a scene.
%
...
\begin{table*}[t]
 \begin{center}
  ...
  \begin{tabular}{|l|l|l|l|l|l|l|} \hline
    \textbf{Streaming Systems} & \textbf{Live/On-demand} & \textbf{2D/3D compression} & \textbf{Content} & \textbf{Uses culling} & \textbf{Bandwidth-adaptive?} & \textbf{FPS} \\ \hline
    ViVo~\cite{vivo} & On-demand & 3D & Multiple Person & \checkmark & \checkmark & 30 \\ \hline
    Groot~\cite{groot} & On-demand & 3D & Full-scene & \checkmark & \crossmark & 30 \\ \hline
    Fumos~\cite{fumos} & On-demand & 3D & Single Person & \checkmark & \crossmark & 30 \\ \hline
    Vues~\cite{vues} & On-demand & 2D & Multiple Person & \checkmark & \checkmark & 30 \\ \hline
    Holoportation~\cite{holoportation} & 2-way Live & 3D & Single Person & \crossmark & \crossmark & 30 \\ \hline
    LiveScan3D~\cite{livescan3d} & 1-way Live & 3D & Full-scene & \crossmark & \crossmark & 15 \\ \hline
    FarFetchFusion~\cite{farfetchfusion} & 1-way Live & 2D & Portrait & \crossmark & \crossmark & 30 \\ \hline
    Starline~\cite{starline} & 2-way Live & 2D & Upper Torso & \crossmark & \crossmark & 30 \\ \hline
    MeshReduce~\cite{meshreduce} & 1-way Live & 3D & Full-scene & \crossmark & \crossmark & 15 \\ \hline
    MetaStream~\cite{metastream} & 1-way Live & 3D & One Person & \crossmark & \crossmark & 30 \\ \hline
    \rowcolor{Gainsboro!60} \sysname & 2-way Live & 2D & Full-scene & \checkmark & \checkmark & 30 \\ \hline
  \end{tabular}}
  \caption{Comparison to related work, more details in \cref{s:related}.}
...
%
For this to be possible, we must transmit full-scene point clouds at \textit{full frame rate} (30 frames per second) with an end-to-end latency of 200-300~ms~\cite{videoconf-benchmark,salsify}; today's conferencing systems are architected to satisfy these requirements.

Full-scene point clouds can be much larger than those that represent more constrained views.
%
For example, in one 3D dataset we use in \cref{s:evaluation}, the average point cloud frame size for a participant is about 1~MB uncompressed, but for the full scene is 10~MB.
%
As such, transmitting full scenes will require significant network bandwidth.
%
Fortunately, average global broadband speeds are already nearly 100~Mbps and are expected to increase by 20\% annually~\cite{cisco_whitepaper}.
%
Thus, we expect immersive two-way live streaming to be feasible in broadband scenarios in the near future, if it is not already.
...
%
In our example above, each full frame needs to be compressed to a third of its original size to fit the 100~Mbps capacity.
%
Moreover, as bandwidth increases over time, we also expect point cloud sizes to grow due to increased scene complexity (\textit{e.g.}, as scenes contain more participants and objects) and possibly increased camera resolution, leading to continued stressing of network resources.
...
However, these techniques can be compute intensive because they construct spatial indices (k-d trees or octrees) for each point cloud for intra-frame (within point cloud) or inter-frame (between successive point clouds) compression.
%
The computational complexity of these compression techniques scale linearly with point cloud size.
%
...
%
Other work reports similar or larger numbers: 100~ms on a 228K point cloud using Draco~\cite{vivo}, and 11-42 minutes using V-PCC for point clouds ranging from 228K-1MB~\cite{groot}.
%
...
%
Parallelism~\cite{vivo,metastream} can help, at the expense of increasing compute cost, which can deter or delay adoption. %\rn{is there a reason we have to keep compute so low given that these are servers?}

%Beyond compression latencies
In addition, point cloud compression techniques do not achieve as high compression efficiencies as 2D video compression.
%
This is because their \textit{inter-frame} compression algorithms are less well developed and the subject of ongoing research~\cite{pcc-future}.
%
For instance, Draco's default setting can compress the 10~MB per full frame video down to about 1.78~MB per frame.
%
In contrast, the technique we describe in this paper, which does not use point cloud compression, can compress this frame down to about 0.66~MB per frame on average, albeit with a small drop (5-10\%) in quality.

\parab{Bandwidth-adaptivity.}
%
Even with increasing average bandwidths, it will be important for two-way live streaming to be \textit{bandwidth-adaptive}.
%
Bandwidth availability can vary spatially (\textit{i.e.}, across different regions) and temporally (\textit{i.e.}, during a single session).
%
Though two-way live streaming can exploit high bandwidth availability to deliver high quality, when bandwidth drops, it must deliver volumetric video at the highest possible quality without stalling.

2D video conferencing systems (e.g., those built using WebRTC~\cite{webrtc}) address this by (a) continuously estimating available bandwidth and (b) using a rate-adaptive codec that compresses a video to the highest possible quality that fits within the bandwidth estimate.
%
To our knowledge, rate-adaptive codecs that compress point clouds directly\footnote{One compression technique, V-PCC, projects point clouds into multiple 2-D planes, then leverages 2D rate-adaptive codes to ensure bandwidth-adaptivity, but it's compute intensive and slow for streaming~\cite{groot}.}
do not exist.


\subsection{\sysname{} Novelty and Approach}
\label{s:approach-1}
%\sanjay{Changed title of this subsec and reworked first few lines.}
In this paper, we present \sysname, a system designed to tackle these challenges. 
%
\sysname focuses on two-way live streaming 
...
%
While some prior work considers two-way streaming, \sysname is the first to support bandwidth-adaptive full-scene streaming. 
%
%In this paper, we describe the design of \sysname, which supports full-scene two-way live volumetric video streaming at full-frame rate with low end-to-end latency.
%
