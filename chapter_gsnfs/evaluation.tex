...
\begin{description}
  ...
  %
  maps Gaussian attributes to 2-D image stacks, one per attribute channel, and compresses the resulting images with an H.264 codec using \texttt{x264} from FFmpeg~\cite{ffmpeg}.
  %
...
Though 2D video codecs are fast, \vthree{}-2D's several hundred milliseconds to encode and decode, due to the overhead of calling the video encoder sequentially for each channel\footnote{\vthree{}-2D encodes and decodes the full scene N3DV dataset faster than the HiFi4G person-centric dataset, because, on the former dataset, QUEEN does not produce degree-3 SHs.};
...
%
In contrast, all other techniques, including \gsnfs{}, only use intra-frame compression, so they have lower compression performance.
...
%
To demonstrate this, we evaluate \gsnfs{}'s decode performance on an NVIDIA Jetson Orin, a mobile GPU with compute comparable to those in AR/VR headsets and edge devices~\cite{nvidia_jetson_orin}.
%
...
%
While \gsnfs's encoder and decoder take only $15$~ms each, the GPS-Gaussian generator takes ${\sim}43$~ms, and is the bottleneck in this pipeline.
...
%
We use six sequences from two datasets: four single person sequences from 8i~\cite{8i-ptcl} (${\sim}700$K--1M points/frame; ${\sim}17$--$26$~MB) and 2 multi-person full-scene sequences from CMU Panoptic~\cite{panoptic} (${\sim}700$K--850K points/frame; ${\sim}17$--$21$~MB).
%
...
%
An efficient implementation of RAHT is crucial for mobile decoding.
%
