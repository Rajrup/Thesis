...
\end{equation}
%
In this equation, $a^0$ represents the low-frequency (coarse) coefficient, $a^1$ the high-frequency coefficient, and $w_l$ and $w_r$ represent \textit{weights} associated with $a_l$ and $a_r$ respectively.
%
...
%
In \gsnfs{}, we use RAHT to encode all \gs{} attributes (\cref{sec:3d-gauss-splat}) other than positions.
%
RAHT decoding inverts this operation; for brevity, we discuss only the RAHT encoder and briefly describe the decoder.
...
%
Then, it reuses these when encoding each attribute.
...
%
The same is true for octree construction (\cref{sec:octree-encode-gpu}), for example, which performs computations only on occupied nodes at each level.
