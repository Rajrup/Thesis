...
Video over the Internet has affected societal and economic transformations.
%
A technology with potentially similar transformative power, 3D video, is on the horizon.
%
In a 3D video, each frame depicts a representation of a scene in three dimensions, and viewers can view the scene from any perspective.
%
...
\subsection{Gaussian Splatting}\label{sec:3d-gauss-splat}

%\parab{Background.}
%
At the core of any 3D video technology is a technique to represent a 3-D scene.
%
While many representations exist, 3-D Gaussian Splats~\cite{gsplat} (or \gs{}) have been the subject of recent research interest.
%
...
\end{itemize}
%
Given a \gs{} model and a viewpoint, a rendering algorithm~\cite{gsplat} produces an image of the view of the scene from the given perspective.
%
...
%
In this paper, we focus on the delivery of \dgs{} content.
%
...
%
Two \textit{complementary} approaches have emerged (\cref{ch:related} discusses related work in detail).
...
%
For example, \vthree{}~\cite{v3volumetric} encodes Gaussian attributes in images and compresses the image sequence using an H.264 codec.
%
...
%
The latter contains a set of points, each with one or more attributes; \gs{} is similar, except that each Gaussian has more attributes (\cref{sec:3d-gauss-splat}).
%
Next, we describe the G-PCC pipeline for encoding \gs{} frames; MesonGS~\cite{mesongs} has used such an approach.

G-PCC's~\cite{gpcc} encoder takes a set of positions and associated attributes as input, then serializes (produces a bitstream) this set for transmission.
%
...
Against this backdrop, \gsnfs{} is, to our knowledge, the first post-compression encoder/decoder for \dgs{} in which \textit{all} components (\cref{fig:architecture_gsnfs}) execute on a GPU and can encode and decode at full frame rate.
%
