...
%
Together, these components made it possible to capture, compress, transport, and render visual information at a global scale.
...
%
A major application in this domain is volumetric telepresence, where multiple participants can interact with each other in a shared virtual space or a real captured space in 6 degrees of freedom (6-DoF).
%
...
%
Commodity RGB-D sensors can capture synchronized color and depth at a high frame rate.
%
...
%
\gsnfs examines frame-rate compression of dynamic Gaussian splats and point clouds, providing a native 3D video codec suitable for emerging live-streaming and on-demand delivery pipelines and perhaps also conferencing in the future.
%
...
%
These results establish that a mature 2D video infrastructure with careful design choices can serve as a practical substrate for immersive two-way communication.
...
\section{Future Work}
\label{sec:intro_v2_future_work}

The work in this dissertation opens several directions for expanding volumetric video beyond the systems studied here. 
%
First, coupling \gsnfs's frame-rate encoder with \livo's network control or developing a separate bitrate control loop for \gsnfs could enable a native 3D delivery pipeline that adapts geometry and appearance directly to available bandwidth. 
%
Second, volumetric communication should grow from two-party sessions to one-to-many and many-to-many settings while accounting for heterogeneous receivers: resource-constrained devices might receive transcoded 2D views, whereas capable headsets and spatial displays could receive native 3D content. 
%
Beyond communication, volumetric streams can support analytics and spatial AI systems that reason about geometry, motion, and interactions in the physical world. 
%
Finally, making interactive 3D objects and scenes first-class 3D web content could enable immersive applications in shopping, social media, sports, and entertainment. 
%
These directions extend the dissertation's contributions from individual conferencing and codec systems toward a broader infrastructure for interactive 3D media and machine understanding.
